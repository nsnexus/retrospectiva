...
export const onRequestGet=async({request,env}:{request:Request;env:Env})=>{
 ...
 if(!origin)return response({error:'A base pública desta região ainda não foi carregada. Não é necessário cadastrar empresas individualmente.'},404);
 ...
 if(q){const terms=sectorTerms(q);where.push('('+terms.map(()=>"search LIKE ? ESCAPE '\\'").join(' OR ')+')');args.push(...terms.map(t=>'%'+t.replace(/[\\%_]/g,'\\$&')+'%'))}
 ...
 return response({companies:matches.slice(0,100),hasMore:matches.length>100,notice:'Base pública da Receita. Distâncias aproximadas entre sedes municipais; não representam o endereço exato. A cobertura se limita às cidades importadas.'});
